    ...
    <div className="w-full">
      ...
      <section className="bg-slate-900 text-white py-16 md:py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-1 text-center md:text-left">
          <span className="text-xs font-bold text-accent-purple uppercase tracking-widest block mb-2">Ghanaian IT Excellence</span>
          <h1 className="text-4xl font-extrabold tracking-tight text-white drop-shadow-md">About Us</h1>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 relative">
            ...
            <div className="bg-theme-surface p-8 rounded-3xl border border-theme-border shadow-xl relative z-1">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  <h4 className="font-extrabold text-theme-heading text-lg leading-tight">Years +</h4>
          ...
          <div className="lg:col-span-7 flex flex-col items-start text-left">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-theme-heading leading-tight mb-6">
            ...
            <div className="space-y-4 w-full">
              <div className="flex items-start space-x-3.5 p-4 bg-theme-surface border border-theme-border rounded-xl">
                <CheckCircle2 className="w-5.5 h-5.5 text-emerald-500 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-bold text-theme-heading text-sm">Cutting-Edge Solutions</h4>
                  <span className="text-xs text-theme-text">We stay ahead of the curve with the latest frameworks and technologies.</span>
                </div>
              </div>
              <div className="flex items-start space-x-3.5 p-4 bg-theme-surface border border-theme-border rounded-xl">
                <CheckCircle2 className="w-5.5 h-5.5 text-emerald-500 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-bold text-theme-heading text-sm">Future-Proofing Your Business</h4>
                  <span className="text-xs text-theme-text">We equip your systems with the tools and strategies to scale with ease.</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="pb-20 bg-theme-surface overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[1, 2, 3, 4, 5, 6].map((num) => (
              <div 
                key={num} 
                className="h-40 md:h-56 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border border-theme-border group"
              >
                <img 
        ...
        <div className="flex whitespace-nowrap space-x-12 animate-marquee">
          {MARQUEE_ITEMS.concat(MARQUEE_ITEMS).map((item, index) => (
            <span key={index} className="text-xs font-bold uppercase tracking-widest inline-flex items-center space-x-2">
              <span>{item}</span>
              <span className="w-1.5 h-1.5 rounded-full bg-theme-surface/50" />
            </span>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {CORE_VALUES.map((val) => (
              <div
                key={val.title}
                className="p-8 rounded-2xl border border-theme-border bg-theme-bg/50 hover:bg-theme-surface hover:shadow-lg transition-all"
              >
                <div className="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-bold text-lg mb-6">
                  {val.title.charAt(0)}
                </div>
                <h3 className="text-lg font-bold text-theme-heading mb-2">{val.title}</h3>
                <p className="text-xs text-theme-text leading-relaxed">{val.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {IMPACTS.map((metric) => (
              <div key={metric.desc} className="text-center flex flex-col items-center">
                ...
                <span className="text-xs text-theme-text">{metric.desc}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 md:py-24 bg-theme-surface border-b border-theme-border">
          ...
          <div className="space-y-4">
            {FAQS.map((faq, index) => {
              ...
              return (
                <div
                  key={index}
                  className="rounded-2xl border border-theme-border hover:border-theme-border transition-colors bg-theme-bg/50"
                >
                  <button
                    onClick={() => toggleFaq(index)}
                    className="w-full flex justify-between items-center px-6 py-5 text-left cursor-pointer focus:outline-none"
                  >
                    <span className="text-sm md:text-base font-bold text-theme-heading">{faq.question}</span>
                    {isOpen ? (
                  ...
                  {isOpen && (
                    <div className="px-6 pb-6 pt-2 border-t border-theme-border/50 animate-in fade-in duration-200">
      ...
      <section className="py-12 bg-theme-bg/50">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {BRANDS.map((brand) => (
              <span key={brand.name} className="inline-flex items-center space-x-1.5 px-4 py-2 border border-theme-border shadow-sm rounded-xl bg-theme-surface text-xs font-semibold text-theme-text">
